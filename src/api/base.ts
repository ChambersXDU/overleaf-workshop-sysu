...
export class BaseAPI {
    ...
    async uploadFile(identity:Identity, projectId:string, parentFolderId:string, filename:string, fileContent:Uint8Array) {
        // NOTE: must use undici's own FormData here — undici's fetch cannot
        // serialize the npm `form-data` stream (it throws / sends no multipart
        // boundary), which used to break every file upload silently.
        ...
        formData.append('qqfile', new Blob([fileContent], {type: mimeType || 'application/octet-stream'}), filename);
    ...
    async uploadProject(identity:Identity, filename:string, fileContent:Uint8Array) {
        const uuid = uuidv4();
        const formData = new FormData();
        formData.append('qqfile', new Blob([fileContent], {type: 'application/zip'}), filename);
